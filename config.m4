...
if test "$PHP_SMAZ" != "no"; then
  ...
  dnl [
  dnl   PHP_ADD_LIBRARY_WITH_PATH($LIBNAME, $SMAZ_DIR/$PHP_LIBDIR, SMAZ_SHARED_LIBADD)
  dnl   AC_DEFINE(HAVE_SMAZLIB,1,[ ])
  dnl ],[
  dnl   AC_MSG_ERROR([wrong smaz lib version or lib not found])
  dnl ],[
  dnl   -L$SMAZ_DIR/$PHP_LIBDIR -lm
  dnl ])
  dnl
  dnl PHP_SUBST(SMAZ_SHARED_LIBADD)

  PHP_NEW_EXTENSION(smaz, smaz.c php_smaz.c, $ext_shared,, -DZEND_ENABLE_STATIC_TSRMLS_CACHE=1)
fi
